<x-app-layout>
    ...
    <div class="flex flex-col w-full text-gray-700 px-2 md:px-8">
        ...
        <div class="w-full text-lg font-semibold">Gerentes</div>
        ...
        <div class="w-full rounded-b-lg bg-white p-3 flex flex-col"> <!--CONTENIDO-->
            <form class="w-full" action="{{route('comision_gerentes',['id'=>$id])}}" class="">
            <input type="hidden" name="filtro" value="ACTIVE"> 
            <div class="w-full flex flex-row space-x-2 bg-slate-400 py-3 px-3">
                    <div class="w-5/6">
                        <x-jet-label class="text-white text-sm">Buscar sucursal/gerente</x-jet-label>
            ...
            <div class="w-full flex justify-center pt-5 flex-col"> <!--TABLA DE CONTENIDO-->
                ...
                ?>
                    <tr class="">
                        <td class="border border-gray-300 font-light {{$color?'bg-orange-100':''}} text-xs px-2 text-orange-500"><a href="{{route('estado_cuenta_gerente',['id'=>$id,'user_id'=>$pago->user_id])}}"><i class="fas fa-balance-scale"></i></a></td>
